 ...
 <div style="margin: 0px 10px 10px 10px">
 	<div class="d-sm-flex align-items-center justify-content-between mb-4">
 		...
 		<a href="{{ route('add_state') }}" class="d-none d-sm-inline-block btn btn-sm font-weight-bold shadow-sm" style="background-color: #ffc107; color: black"><i class="fas fa-fw fa-1x fa-plus fa-sm text-dark-300"></i>Add New State</a>
 	</div>
 		...
 		<table class="table table-striped table-hover" id="state_list" style=" font-size: 13px">
 			...
 			<tbody>
 				...
 				<tr style="color: black">
 					...
 					<td>{{$state->name}}</td>
 					<td>
 						<a href="{{ route('state.status', $state->id) }}" class="btn {{ ($state->status === 1) ? 'btn-success' : 'btn-danger' }}" style="color: white;width:6rem">
